...
file_env() {
	...
	#def -> $2가 있으면 $2, 없으면 없음.
	local def="${2:-}"
	# var, fileVar 둘 다 있으면 에러 (! 는 var 에 저장된 걸 환경변수 취급)
	if [ "${!var:-}" ] && [ "${!fileVar:-}" ]; then
	...
	export "$var"="$val"
	unset "$fileVar"  #왜 굳이 언셋을?
}

# set MARIADB_xyz from MYSQL_xyz when MARIADB_xyz is unset
# and make them the same value (so user scripts can use either)
_mariadb_file_env() {
	# $1 -> MYSQL_ROOT_HOST 같은 거.. $2 -> % (MYSQL_ROOT_HOST 만 있음)
	local var="$1"; shift # 저장 후 버림
	local maria="MARIADB_${var#MYSQL_}" # MARIADB_ROOT_HOST
	# 또 함수;
	file_env "$var" "$@"
	file_env "$maria" "${!var}"
	if [ "${!maria:-}" ]; then
...
mysql_check_config() {
	# NOTE: toRun -> mariadbd --verbose --help 또는 mysqld --help --verbose
	local toRun=( "$@" "${_verboseHelpArgs[@]}" ) errors
	# errors 에 stderr 아웃풋 저장하고 (2>&1) 원래 stdout 은 버림 (/dev/null). 실패하면 에러 출력 (!)
...
mysql_get_config() {
	# first time $1 is datadir, second time it's socket.
	# NOTE: shift 로 $1 을 버리고 $2 를 $1 로 옮김
	local conf="$1"; shift
	# 여기서 써야댐
...
docker_create_db_directories() {
  ...
  # root
	if [ "$user" = "0" ]; then
		# this will cause less disk access than `chown -R` 헤이 너네 또라이니?
...
docker_setup_env() {
	# Get config
	# NOTE: declare 하면 전역변수, local 하면 지역변수.
...
_main() {
	...
	# NOTE: parameter:offset:length -> if the first paratmeter($1) is started with '-'
	# then set the first parameter to 'mariadbd'
	if [ "${1:0:1}" = '-' ]; then
		set -- mariadbd "$@"
	fi
	...
	# NOTE : 첫 인자가 mariadbd 또는 mysqld이고, -?, --help, --print-defaults, -V, --version 중 하나가 아니라면
	# $@ 는 모든 positional parameter를 의미한다.
